...
### Fixed
- NEAR operator now works with lowercase: `near(api key)` normalized to `NEAR(api key)`
- Infix NEAR syntax now works: `api NEAR key` converted to `NEAR(api key)` (FTS5 only supports function form)
- AND, OR, NOT operators normalized to uppercase for FTS5 compatibility

### Added
- FTS5 syntax validation test suite (49 tests) covering all advertised F1 panel features
- `make test-syntax` target for running FTS5 syntax tests
